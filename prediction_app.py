...
flat_address = st.sidebar.text_input(
    "Flat Address or Postal Code", '212 Tampines Street 23')  # flat address
...
flat_type = st.sidebar.selectbox('Flat Type', list(['2 ROOM', '3 ROOM', '4 ROOM', '5 ROOM', 'EXECUTIVE']),
                                 index=1)
floor_area = st.sidebar.slider("Floor Area (sqm)", 34,280,78) # floor area
...
lease_commence_date = st.sidebar.selectbox('Lease Commencement Date', list(reversed(range(1966, 2020))), index=1)
...
def load_model():
    ...
    f_checkpoint = Path("rf_compressed.pkl")
    f_checkpoint1 = Path("shap_explainer.pkl")
...
supermarket_coord = load_data('Data_app/supermarket_coordinates_clean.csv')
school_coord = load_data('Data_app/school_coordinates_clean.csv')
hawker_coord = load_data('Data_app/hawker_coordinates_clean.csv')
shop_coord = load_data('Data_app/shoppingmall_coordinates_clean.csv')
park_coord = load_data('Data_app/parks_coordinates_clean.csv')
mrt_coord = load_data('Data_app/MRT_coordinates.csv')[['STN_NAME','Latitude','Longitude']]
...
flat_coord['dist_cityhall'] = [list(dist_cityhall.values())[0][1]]
...
flat1 = flat_coord[['flat_type', 'storey_range', 'floor_area_sqm', 'lease_commence_date',
       ...
       'num_mrt_2km', 'supermarket_dist', 'num_supermarket_2km', 'dist_cityhall',
...
## MAP OF USER HDB ====================================================================================
flats = pd.read_csv('Data_app/flat_coordinates_clean.csv')[['LATITUDE','LONGITUDE','address']]
...
The machine learning model that is used for this resale price prediction is a **random forest model**. It was trained on
                HDB resale prices data from 2016 to 2021. The data was split into a 9:1 train test ratio, and validated using 10-fold cross validation, 
                achieving a **test Rsquare of 0.93** and **mean absolute error of ~$24,000**.""")
